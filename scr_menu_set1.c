...
void ScrMenu_Set1_Update()
{
	...
	if (!scr_menu_init)
	{
		...
		DataTextLCD_I_L(&menu_lines_main[MENU_MAIN_SET1 * 20], 16, COLOR_WHITE, zt_main_bg, MENU_SCR_TITLE_X, MENU_SCR_TITLE_Y);
	...
	if (scr_menu_line_sel != scr_menu_line_sel_1 || !scr_menu_init)
	{
		...
		for (i = scr_menu_line_start; i < (((scr_menu_line_start + 7) <= menu_count_set1) ? (scr_menu_line_start + 7) : menu_count_set1); i++)
		{
...
{
	ScrMenu_Start(scr_menu_main, MENU_MAIN_SET1 - 1);
}
...
{
	if (scr_menu_line_sel < menu_count_set1 - 1)
		scr_menu_line_sel++;
}
